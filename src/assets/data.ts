import { Project } from '../components/AddNewProject/Modal/Modal';

export const dataBase: Project[] = [
  {
    id: '1',
    name: 'Facebook',
    description:
      'This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original. This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original. This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original. /n- Do that thing preety nice ,  /n-- Do that thing preety nice /n- Do that thing preety nice /n- Do that thing preety nice',
    framework: 'react',
    date: new Date(),
    level: 'Beginner',
    team: [{ name: 'frontend dev' }, { name: 'frontend dev' }],
  },
  {
    id: '2',
    name: 'Instagram',
    description:
      'This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original. This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original. This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original. /n- Do that thing preety nice ,  /n-- Do that thing preety nice /n- Do that thing preety nice /n- Do that thing preety nice',
    framework: 'vue',
    date: new Date(),
    level: 'Intermediate',
    team: [{ name: 'frontend dev' }, { name: 'frontend dev' }, { name: 'frontend dev' }],
  },
  {
    id: '3',
    name: 'Portfolio',
    description:
      'This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original. This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original. This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original. /n- Do that thing preety nice ,  /n-- Do that thing preety nice /n- Do that thing preety nice /n- Do that thing preety nice',
    framework: 'angular',
    date: new Date(),
    level: 'Senior',
    team: [{ name: 'frontend dev' }, { name: 'backend dev' }, { name: 'ui designer' }],
  },
  {
    id: '4',
    name: 'Facebook',
    description:
      'This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original. This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original. This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original. /n- Do that thing preety nice ,  /n-- Do that thing preety nice /n- Do that thing preety nice /n- Do that thing preety nice',
    framework: 'react',
    date: new Date(),
    level: 'Beginner',
    team: [{ name: 'frontend dev' }, { name: 'backend dev' }, { name: 'ui designer' }],
  },
  {
    id: '5',
    name: 'Facebook',
    description:
      'This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original. This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original. This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original. /n- Do that thing preety nice ,  /n-- Do that thing preety nice /n- Do that thing preety nice /n- Do that thing preety nice',
    framework: 'react',
    date: new Date(),
    level: 'Beginner',
    team: [{ name: 'frontend dev' }, { name: 'backend dev' }, { name: 'ui designer' }],
  },
  {
    id: '6',
    name: 'Portfolio',
    description:
      'This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original. This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original. This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original. /n- Do that thing preety nice ,  /n-- Do that thing preety nice /n- Do that thing preety nice /n- Do that thing preety nice',
    framework: 'angular',
    date: new Date(),
    level: 'Senior',
    team: [{ name: 'frontend dev' }, { name: 'backend dev' }, { name: 'ui designer' }],
  },
  {
    id: '7',
    name: 'Facebook',
    description:
      'This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original. This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original. This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original. /n- Do that thing preety nice ,  /n-- Do that thing preety nice /n- Do that thing preety nice /n- Do that thing preety nice',
    framework: 'react',
    date: new Date(),
    level: 'Beginner',
    team: [{ name: 'frontend dev' }, { name: 'frontend dev' }],
  },
  {
    id: '8',
    name: 'Facebook',
    description:
      'This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original.',
    framework: 'react',
    date: new Date(),
    level: 'Beginner',
    team: [{ name: 'frontend dev' }, { name: 'frontend dev' }],
  },
  // {
  // 	id: 9,
  // 	name: "Portfolio",
  // 	description:
  // 		"This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original.",
  // 	framework: "Angular",
  // 	date: new Date(),
  // 	level: "Senior",
  // 	team: [{ name: "frontend dev" }, { name: "frontend dev" }, { name: "frontend dev" }],
  // },
  // {
  // 	id: 10,
  // 	name: "Facebook",
  // 	description:
  // 		"This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original.",
  // 	framework: "React",
  // 	date: new Date(),
  // 	level: "Beginner",
  // 	team: [{ name: "frontend dev" }, { name: "frontend dev" }],
  // },
  // {
  // 	id: 11,
  // 	name: "Facebook",
  // 	description:
  // 		"This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original.",
  // 	framework: "React",
  // 	date: new Date(),
  // 	level: "Beginner",
  // 	team: [{ name: "frontend dev" }, { name: "frontend dev" }],
  // },
  // {
  // 	id: 12,
  // 	name: "Facebook",
  // 	description:
  // 		"This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original.",
  // 	framework: "React",
  // 	date: new Date(),
  // 	level: "Beginner",
  // 	team: [{ name: "frontend dev" }, { name: "frontend dev" }],
  // },
  // {
  // 	id: 13,
  // 	name: "Facebook",
  // 	description:
  // 		"This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original.",
  // 	framework: "React",
  // 	date: new Date(),
  // 	level: "Beginner",
  // 	team: [{ name: "frontend dev" }, { name: "frontend dev" }],
  // },
  // {
  // 	id: 14,
  // 	name: "Facebook",
  // 	description:
  // 		"This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original.",
  // 	framework: "React",
  // 	date: new Date(),
  // 	level: "Beginner",
  // 	team: [{ name: "frontend dev" }, { name: "frontend dev" }],
  // },
  // {
  // 	id: 15,
  // 	name: "Facebook",
  // 	description:
  // 		"This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original.",
  // 	framework: "React",
  // 	date: new Date(),
  // 	level: "Beginner",
  // 	team: [{ name: "frontend dev" }, { name: "frontend dev" }],
  // },
  // {
  // 	id: 16,
  // 	name: "Facebook",
  // 	description:
  // 		"This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original.",
  // 	framework: "React",
  // 	date: new Date(),
  // 	level: "Beginner",
  // 	team: [{ name: "frontend dev" }, { name: "frontend dev" }],
  // },
  // {
  // 	id: 17,
  // 	name: "Facebook",
  // 	description:
  // 		"This is a project that we want to re-cerate an app that look like facebook page, add some chat, posts, and other stuff similar to original.",
  // 	framework: "React",
  // 	date: new Date(),
  // 	level: "Beginner",
  // 	team: [{ name: "frontend dev" }, { name: "frontend dev" }],
  // },
];
